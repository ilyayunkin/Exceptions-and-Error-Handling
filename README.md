...
Хотя возврат кодов ошибок **иногда** есть наиболее правильный способ обработки ошибок, есть побочные эффекты от ввода дополнительных условных операторов:
...
### Я все еще не убежден: фрагмент кода в 4 строки показывает, что коды ошибок ничем не хуже исключений. Почему я должен использовать исключения на более крупных проектах?
...
Механизм исключений в С++ может быть мощным и полезным, но если у вас есть неверные представления, результат может быть плох. Это инструмент. Используйте его по назначению и он вам поможет, но не обвиняйте инструмент , если применяете его не по назначению.
...
Напишите в лог. Завершите процесс. НО НЕ БРОСАЙТЕ ИСКЛЮЧЕНИЕ!
...
Если конструктор бросает исключение, деструктор вызван не будет. Если ваш код уже сделал что-то, что должно быть  почищено (например, выделил память, открыл файл, заблокировал семафор), это "что-то" должно быть запомнено каким-то членом данных объекта.
...
### Как изменять длину строки массива символов, чтобы не допустить утечки памяти даже если кто-то бросит исключение?
Если вы хотите работать со строками, не используйте массивы. Используйте какой-либо строковый класс.
...
```
Работать так с указателями утомительно и черевато ошибками. Почему бы просто не использовать  строковый класс? Вероятно, ваш компилятор предоставляет строковый класс, который, вероятно, так же быстр, и он определенно проще и безопаснее, чем самописный код с указателями. Если вы используете std::string, то получет примерно такой код:
```
...
Если вы создаете свой фреймворк или часть системы, которая не зависит от MFC напрямую, то не следует ловить по указателю только из уважения к MFC. Библиотеки вроде MFC  руководствуются соображениями обратной совместимости со старым кодом, которые заставляют ловить по указателю.
...
Потому что С++ имеет альтернативу, которая работает лучше почти весгда: RAII. Основная идея в том, чтобы представить ресурсы локальным объектом, чтобы деструктор освобождал ресурсы. Тогда программист не может забыть освободить ресурсы:
```
...
```
В худшем случае у нас будет столько же классов управления ресурсами, сколько и ресурсов. Однако в реальной жизни выделений ресурсов гораздо больше, чем типов ресурсов, так что RAII порождает меньше кода, чем  “finally”.
...
Нельзя быть уверенным, что код, расположенный за местом возникновения исключения готов работать дальше. Авторам бросающего и ловящего кода следовало бы хорошо знать код друг друга, порождая сложную взаимосвязь кода, которая создала бы серьезные проблемы на этапе сопровождения.
